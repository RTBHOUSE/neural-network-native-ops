...
public final class NeuralNetworkNativeOps {
    ...
    public static void gemm(FloatBuffer A, FloatBuffer B, FloatBuffer Y) {
        gemm(A, B, Y,
                (int) Math.sqrt(A.capacity() * Y.capacity() / B.capacity()),
                (int) Math.sqrt(A.capacity() * B.capacity() / Y.capacity()),
                (int) Math.sqrt(B.capacity() * Y.capacity() / A.capacity()));
    }
     ...
     * @param input         input matrix with size {@code inputRowSize} x {@code batchSize} (ro). Values from one row should occupy consecutive memory
     *                      cells
